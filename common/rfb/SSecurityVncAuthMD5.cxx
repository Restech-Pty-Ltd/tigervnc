...
#include <rfb/SSecurityVncAuthMD5.h>
...
static LogWriter vlog("SVncAuthMD5");

StringParameter SSecurityVncAuthMD5::vncMD5AuthPasswdFile
("MD5PasswordFile", "MD5 Password file for VNC authentication", "", ConfServer);
AliasParameter MD5rfbauth("MD5rfbauth", "Alias for PasswordFile",
		       &SSecurityVncAuthMD5::vncMD5AuthPasswdFile, ConfServer);
VncAuthPasswdParameterMD5 SSecurityVncAuthMD5::vncAuthPasswd
("Password", "Obfuscated binary encoding of the password which clients must supply to "
 "access the server", &SSecurityVncAuthMD5::vncMD5AuthPasswdFile);

SSecurityVncAuthMD5::SSecurityVncAuthMD5(SConnection* sc)
  : SSecurity(sc), sentChallenge(false),
    pg(&vncAuthPasswd), accessRights(AccessNone)
{
}

bool SSecurityVncAuthMD5::verifyResponse(const char* password)
{
...
bool SSecurityVncAuthMD5::processMsg()
{
...
VncAuthPasswdParameterMD5::VncAuthPasswdParameterMD5(const char* name,
                                               const char* desc,
                                               StringParameter* passwdFile_)
: BinaryParameter(name, desc, 0, 0, ConfServer), passwdFile(passwdFile_) {
}

void VncAuthPasswdParameterMD5::getVncAuthPasswd(std::string *password, std::string *readOnlyPassword) {
